...
public class MoleculesFactory {
    ...
    public Molecule buildNewMolecule() {
        ...
        for (int i = 0; i <= random(min); i++) {
            ...
            if (probabilityFunction(functionalGroupNodes, aProba)) {
                Integer mainGroupCode = CamdRunner.CONTRIBUTION_GROUPS.getThermoPhysicalFirstOrderContributions().get(groupCode).getMainGroup().getCode();
                opt = mainGroupCode <= 1;
            } else
                opt = false;
        }
    ...
    private Molecule newMoleculeFromBaseGroups(ArrayList<ContributionGroupNode> leaves, int dim) {
        ...
        int groupValence = CamdRunner.CONTRIBUTION_GROUPS.getThermoPhysicalFirstOrderContributions().get(group.getGroupCode()).getValence();
        ...
        if (leaves.size() <= groupValence) { // if is the last group
            ...
            int valence = CamdRunner.CONTRIBUTION_GROUPS.getThermoPhysicalFirstOrderContributions().get(group.getGroupCode()).getValence();
            ...
            while (next) {
                if (leaves.size() > 0) {
                    ...
                    if (CamdRunner.CONTRIBUTION_GROUPS.getThermoPhysicalFirstOrderContributions().get(group.getGroupCode()).getValence() - 1 == group.getSubGroups().size())
                        next = false; //
                } else {
                    next = false;
                }
