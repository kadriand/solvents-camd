...
public class DielectricConstant {
    ...
    public DielectricConstant(Molecule molecule) {
    ...
    private double computeVaporizationHeat() {
        ...
        for (Map.Entry<ContributionGroup, Integer> firstOrderContributionEntry : molecule.getFirstOrderContributions().entrySet())
            c += firstOrderContributionEntry.getValue() * firstOrderContributionEntry.getKey().getThermoPhysicalFirstContribution().getDipoleMomentH1i();
        return c;
    }
    ...
    private double computeMolarVolume() {
        ...
        for (Map.Entry<ContributionGroup, Integer> firstOrderContributionEntry : molecule.getFirstOrderContributions().entrySet())
            c += firstOrderContributionEntry.getValue() * firstOrderContributionEntry.getKey().getThermoPhysicalFirstContribution().getLiquidMolarVolume();
    ...
    private double computeD() {
        double R = 8.314;
        double d = Math.pow((computeVaporizationHeat() - R * ProblemParameters.getTemperature() / 1000) / computeMolarVolume(), 0.5);
        return d;
    }
    ...
    private double computeDipolarMoment(boolean conditionCase) {
        ...
        for (Map.Entry<ContributionGroup, Integer> firstOrderContributionEntry : molecule.getFirstOrderContributions().entrySet())
            if (IntStream.of(conditionGHC).anyMatch(i -> i == firstOrderContributionEntry.getKey().getCode()))
        ...
        double c = 0;
        for (Map.Entry<ContributionGroup, Integer> firstOrderContributionEntry : molecule.getFirstOrderContributions().entrySet())
            c += firstOrderContributionEntry.getValue() * firstOrderContributionEntry.getKey().getThermoPhysicalFirstContribution().getDipoleMoment();
    ...
    private double computeE1(boolean conditionCase, boolean otherCond) {
        ...
        int sum = 0;
        for (Map.Entry<ContributionGroup, Integer> firstOrderContributionEntry : molecule.getFirstOrderContributions().entrySet())
            if (IntStream.of(condition1_9).anyMatch(i -> i == firstOrderContributionEntry.getKey().getCode()))
                sum += firstOrderContributionEntry.getValue();
    ...
    private boolean isConditionCase(int[] conditionCase) {
        for (Map.Entry<ContributionGroup, Integer> firstOrderContributionEntry : molecule.getFirstOrderContributions().entrySet())
            if (IntStream.of(conditionCase).anyMatch(i -> i == firstOrderContributionEntry.getKey().getCode()))
                return true;
        return false;
    }
