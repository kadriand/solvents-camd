    ...
    public static final double compute(Molecule molecule) {
        ...
        for (ContributionGroupNode contributionGroup : molecule.pickAllGroups()) {
            ...
            if (isBond(contributionGroup, 10, 4)) {
                ...
                for (int j = 0; j < 4; j++) {
                    a = firstOrderContribution.getThermoPhysicalFirstContribution().getDensityA()[j];
                    b = firstOrderContribution.getThermoPhysicalFirstContribution().getDensityB()[j];
                    c = firstOrderContribution.getThermoPhysicalFirstContribution().getDensityC()[j];
                    sum += (a + b * temperature + c * temperature * temperature);
                }
            }
        }
